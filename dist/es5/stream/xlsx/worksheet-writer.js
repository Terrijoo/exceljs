...
class WorksheetWriter {
  ...
  _writeSheetIgnoredErrors(xmlBuf, ignoredErrors) {
    if (ignoredErrors.length) {
      xmlBuf.addText('<ignoredErrors>');
      for (let i = 0; i < ignoredErrors.length; i++) {
        xmlBuf.addText("<ignoredError sqref=\"".concat(ignoredErrors[i].sqref, "\" ").concat(ignoredErrors[i].type, "=\"").concat(ignoredErrors[i].value, "\"/>'"));
      }
    }
  }
  _writeOpenWorksheet() {
    ...
    this._writeSheetIgnoredErrors(xmlBuffer, this.ignoredErrors);
    this.stream.write(xmlBuffer);
  }
  _writeColumns() {
